...
document.addEventListener("DOMContentLoaded", function () {
    ...
    uniqueBrands.forEach(brand => {
        ...
        brandItem.className = "cursor-pointer py-2 px-3 rounded-lg hover:bg-white hover:shadow";
        brandItem.textContent = brand;

        brandItem.addEventListener("click", function () {
            displayProducts(allProducts.filter(p => p.brand === brand));
        });
    ...
    function displayProducts(productArray) {
        ...
        productArray.forEach(product => {
            ...
            card.href = "#";
            card.className = "group relative bg-white rounded-2xl shadow-sm hover:shadow-lg transition";
            card.innerHTML = `
                <button class="absolute top-3 right-3 z-10 bg-white/80 backdrop-blur-sm rounded-full p-2 shadow hover:text-red-500">
                    <i class="fa-regular fa-heart"></i>
                </button>
                <div class="bg-gray-100 p-6">
                    <img src="${product.thumbnail}"
                         class="h-48 w-full object-contain group-hover:scale-105 transition">
                </div>
